...
- localStorage 通常有 5MB 左右的容量限制，成員大頭貼會自動壓縮，但仍建議定期清理不需要的舊旅程。雲端同步每趟旅程各自存成一份 Firestore 文件，單一份文件約有 1MB 上限——單一趟旅程的大頭貼太多可能導致「那一趟旅程」同步失敗（本機資料不受影響，其他旅程仍會正常同步，畫面上也會清楚顯示是哪一趟旅程超過上限）。
